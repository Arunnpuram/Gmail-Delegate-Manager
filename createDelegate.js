// Script to Give delegated access to a user account for a shared mailbox via google SDK
...
async function addDelegateAccount() {
    try {
        ...
        const userEmail = await askQuestion('Enter the email of the inbox to impersonate: ');
        // Prompt for the email of the delegate
        const delegateEmail = await askQuestion('Enter the email of the delegate: ');
        ...
        console.log('Checking if delegate already exists...');
        ...
        console.log('Delegate account added successfully.');
        console.log('Response:', res.data);
    } catch (error) {
        console.error('Error adding user account as delegate:', error);
        if (error.response) {
